# Hi, I’m Samuel

I’m a technical writer focused on creating clear, structured, and user-friendly documentation for software and digital products.

## What I’m Working On

- Improving open-source documentation on GitHub
- Building sample product documentation (GoCarte project)
- Developing skills in UX writing and information architecture

## Featured Work

### readme-radar
https://github.com/samuelfatzinger/readme-radar

CLI tool that finds GitHub repositories with weak or missing README files to identify documentation contribution opportunities.

### GoCarte Product Documentation
https://github.com/samuelfatzinger/gocarte-product-documentation

A full set of user, admin, and developer documentation for a SaaS-style project.

### Open Source Contributions

Improving existing READMEs by restructuring content, clarifying onboarding steps, and making documentation more accessible to new users.


## Skills

- Technical writing
- Documentation structure and organization
- Information architecture
- UX writing

## Contact

- GitHub: [github.com/samuelfatzinger](https://github.com/samuelfatzinger)
- LinkedIn: [linkedin.com/in/samuel-fatzinger-b581a92ba](https://linkedin.com/in/samuel-fatzinger-b581a92ba)

## Blog

- dev.to: [dev.to/samuelfatzinger](dev.to/samuelfatzinger)
